...
class AgentRunner:
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.finish_reason != "error" and is_blank_text(clean):
                ...
                try:
                    ...
                    logger.warning(
                        "Finalization retry failed on turn {} for {}: {}",
                        iteration,
                        spec.session_key or "default",
                        exc,
                    )
    ...
    async def _run_tool(
        ...
        try:
            execute = getattr(spec.tools, "execute", None)
            execute_override = getattr(getattr(spec.tools, "__dict__", {}), "get", lambda _k, _d=None: None)(
                "execute",
                None,
            )
            if callable(execute_override):
                ...
                raise RuntimeError(f"Tool '{tool_call.name}' is not executable")
