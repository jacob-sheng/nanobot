...
def test_no_recent_history_when_dream_has_processed_all(tmp_path) -> None:
    ...
    cursor = builder.memory.append_history("old conversation about Python")
    builder.memory.append_history("recent question about Docker")
    builder.memory.set_last_dream_cursor(cursor + 1)
...
def test_recent_history_skips_non_durable_workflow_entries(tmp_path) -> None:
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)

    builder.memory.append_history("normal durable memory candidate")
    builder.memory.append_history("[SCRUBBED_NON_DURABLE_WORKFLOW] Removed a transient scheduled-workflow report")

    prompt = builder.build_system_prompt()
    assert "normal durable memory candidate" in prompt
    assert "SCRUBBED_NON_DURABLE_WORKFLOW" not in prompt
...
def test_runtime_context_uses_utc8_for_shanghai_timezone(tmp_path) -> None:
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace, timezone="Asia/Shanghai")
    ...
    assert "UTC+08:00" in user_content
